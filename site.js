const revealItems = document.querySelectorAll(".reveal");

if ("IntersectionObserver" in window && revealItems.length) {
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (!entry.isIntersecting) {
        return;
      }

      entry.target.classList.add("visible");
      observer.unobserve(entry.target);
    });
  }, { threshold: 0.12, rootMargin: "0px 0px -40px 0px" });

  revealItems.forEach((item) => observer.observe(item));
} else {
  revealItems.forEach((item) => item.classList.add("visible"));
}

const nav = document.getElementById("nav");

if (nav) {
  const syncNavState = () => {
    nav.classList.toggle("scrolled", window.scrollY > 40);
  };

  syncNavState();
  window.addEventListener("scroll", syncNavState, { passive: true });
}

const hamburger = document.getElementById("hamburger");
const drawer = document.getElementById("navDrawer");

if (hamburger && drawer) {
  const drawerInner = drawer.querySelector(".nav-drawer-inner");
  const focusableSelector = [
    "a[href]",
    "button:not([disabled])",
    "[tabindex]:not([tabindex='-1'])",
  ].join(", ");
  let previousFocus = null;

  const getFocusableItems = () => {
    const scope = drawerInner || drawer;
    return Array.from(scope.querySelectorAll(focusableSelector)).filter(
      (element) => !element.hasAttribute("disabled") && element.getAttribute("aria-hidden") !== "true",
    );
  };

  const openDrawer = () => {
    previousFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    drawer.classList.add("open");
    hamburger.classList.add("open");
    hamburger.setAttribute("aria-expanded", "true");
    drawer.setAttribute("aria-hidden", "false");
    drawer.setAttribute("aria-modal", "true");
    document.body.style.overflow = "hidden";

    const firstFocusable = getFocusableItems()[0];
    window.setTimeout(() => {
      (firstFocusable || drawerInner || drawer).focus();
    }, 20);
  };

  const closeDrawer = ({ restoreFocus = true } = {}) => {
    drawer.classList.remove("open");
    hamburger.classList.remove("open");
    hamburger.setAttribute("aria-expanded", "false");
    drawer.setAttribute("aria-hidden", "true");
    drawer.removeAttribute("aria-modal");
    document.body.style.overflow = "";

    if (restoreFocus && previousFocus) {
      previousFocus.focus();
    }
  };

  if (drawerInner && !drawerInner.hasAttribute("tabindex")) {
    drawerInner.setAttribute("tabindex", "-1");
  }

  hamburger.addEventListener("click", () => {
    drawer.classList.contains("open") ? closeDrawer() : openDrawer();
  });

  drawer.addEventListener("click", (event) => {
    if (event.target === drawer) {
      closeDrawer();
    }
  });

  drawer.querySelectorAll("a").forEach((link) => {
    link.addEventListener("click", () => closeDrawer({ restoreFocus: false }));
  });

  document.addEventListener("keydown", (event) => {
    if (!drawer.classList.contains("open")) {
      return;
    }

    if (event.key === "Escape") {
      event.preventDefault();
      closeDrawer();
      return;
    }

    if (event.key !== "Tab") {
      return;
    }

    const focusableItems = getFocusableItems();

    if (!focusableItems.length) {
      return;
    }

    const firstItem = focusableItems[0];
    const lastItem = focusableItems[focusableItems.length - 1];

    if (event.shiftKey && document.activeElement === firstItem) {
      event.preventDefault();
      lastItem.focus();
      return;
    }

    if (!event.shiftKey && document.activeElement === lastItem) {
      event.preventDefault();
      firstItem.focus();
    }
  });
}

const whatsappNumber = "5579991373093";
const pricingData = window.PRICING_DATA;

const onlyDigits = (value) => value.replace(/\D/g, "");

const formatPhone = (value) => {
  const digits = onlyDigits(value).slice(0, 11);

  if (digits.length > 6) {
    return `(${digits.slice(0, 2)}) ${digits.slice(2, 7)}-${digits.slice(7)}`;
  }

  if (digits.length > 2) {
    return `(${digits.slice(0, 2)}) ${digits.slice(2)}`;
  }

  return digits;
};

const fieldValue = (form, name) => {
  const field = form.elements[name];
  return field ? field.value.trim() : "";
};

const setFieldState = (form, name, message) => {
  const field = form.elements[name];
  const error = form.querySelector(`[data-error-for="${name}"]`);

  if (!field) {
    return;
  }

  const hasError = Boolean(message);
  field.classList.toggle("error", hasError);
  field.setAttribute("aria-invalid", hasError ? "true" : "false");

  if (error) {
    if (message) {
      error.textContent = message;
    }
    error.classList.toggle("show", hasError);
  }
};

const normalizeFragment = (value) =>
  value
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");

const isVisibleServiceGroup = (groupLabel) => {
  const label = (groupLabel || "").trim();

  if (!label) {
    return false;
  }

  const normalized = label.toLocaleLowerCase("pt-BR");
  return !label.startsWith("⚠") && !normalized.includes("preços referentes");
};

const hydrateServiceSelects = () => {
  if (!pricingData?.groups?.length) {
    return;
  }

  const groupedOptions = pricingData.groups.reduce((map, entry) => {
    const area = entry.sheet;
    const label = (entry.group || "").trim();

    if (!label) {
      return map;
    }

    if (!map.has(area)) {
      map.set(area, new Set());
    }

    map.get(area).add(label);
    return map;
  }, new Map());

  document.querySelectorAll("[data-service-select]").forEach((select) => {
    if (!(select instanceof HTMLSelectElement)) {
      return;
    }

    const currentValue = select.value.trim();
    select.innerHTML = "";

    const placeholder = document.createElement("option");
    placeholder.value = "";
    placeholder.textContent = "Selecione uma área ou categoria...";
    select.appendChild(placeholder);

    Array.from(groupedOptions.entries())
      .sort((a, b) => a[0].localeCompare(b[0], "pt-BR"))
      .forEach(([area, groups]) => {
        const optgroup = document.createElement("optgroup");
        optgroup.label = area;

        Array.from(groups)
          .sort((a, b) => a.localeCompare(b, "pt-BR"))
          .forEach((group) => {
            const option = document.createElement("option");
            option.value = `${area} · ${group}`;
            option.textContent = group;
            optgroup.appendChild(option);
          });

        select.appendChild(optgroup);
      });

    const guidanceOption = document.createElement("option");
    guidanceOption.value = "Ainda não sei / preciso de orientação";
    guidanceOption.textContent = "Ainda não sei / preciso de orientação";
    select.appendChild(guidanceOption);

    if (currentValue) {
      const matchingOption = Array.from(select.options).find((option) => option.value === currentValue);

      if (matchingOption) {
        select.value = currentValue;
      }
    }
  });
};

const buildServicesCatalog = () => {
  if (!pricingData?.items?.length || !pricingData?.areas?.length) {
    return [];
  }

  const visibleItems = pricingData.items.filter((item) => isVisibleServiceGroup(item.group));
  const orderedGroupsByArea = pricingData.groups.reduce((map, entry) => {
    if (!isVisibleServiceGroup(entry.group)) {
      return map;
    }

    if (!map.has(entry.sheet)) {
      map.set(entry.sheet, []);
    }

    map.get(entry.sheet).push(entry.group);
    return map;
  }, new Map());

  return pricingData.areas
    .map((area) => {
      const areaItems = visibleItems.filter((item) => item.sheet === area);

      if (!areaItems.length) {
        return null;
      }

      const itemsByGroup = areaItems.reduce((map, item) => {
        if (!map.has(item.group)) {
          map.set(item.group, []);
        }

        map.get(item.group).push(item);
        return map;
      }, new Map());

      const preferredOrder = orderedGroupsByArea.get(area) || [];
      const orderedGroupNames = [
        ...preferredOrder.filter((groupName) => itemsByGroup.has(groupName)),
        ...Array.from(itemsByGroup.keys())
          .filter((groupName) => !preferredOrder.includes(groupName))
          .sort((a, b) => a.localeCompare(b, "pt-BR")),
      ];

      return {
        name: area,
        slug: areaItems[0].sheetSlug || normalizeFragment(area),
        totalItems: areaItems.length,
        previewItems: areaItems.slice(0, 5).map((item) => item.description),
        groups: orderedGroupNames.map((groupName) => {
          const groupItems = itemsByGroup.get(groupName) || [];

          return {
            name: groupName,
            slug: groupItems[0]?.groupSlug || normalizeFragment(groupName),
            totalItems: groupItems.length,
            preview: groupItems.slice(0, 3).map((item) => item.description),
          };
        }),
      };
    })
    .filter(Boolean);
};

const hydrateServicesCatalog = () => {
  const catalog = document.querySelector("[data-services-catalog]");

  if (!catalog) {
    return;
  }

  const areas = buildServicesCatalog();

  if (!areas.length) {
    return;
  }

  const totalServices = document.querySelector("[data-services-total]");
  const totalAreas = document.querySelector("[data-services-areas]");
  const jumpList = document.querySelector("[data-services-jump]");

  if (totalServices) {
    totalServices.textContent = String(areas.reduce((sum, area) => sum + area.totalItems, 0));
  }

  if (totalAreas) {
    totalAreas.textContent = String(areas.length);
  }

  if (jumpList) {
    jumpList.innerHTML = "";

    areas.forEach((area) => {
      const link = document.createElement("a");
      link.className = "services-jump-chip";
      link.href = `#services-area-${normalizeFragment(area.slug)}`;
      link.textContent = area.name;
      jumpList.appendChild(link);
    });
  }

  const fragment = document.createDocumentFragment();

  areas.forEach((area) => {
    const card = document.createElement("article");
    card.className = "services-area-card visible";
    card.id = `services-area-${normalizeFragment(area.slug)}`;

    const header = document.createElement("div");
    header.className = "services-area-head";

    const headingWrap = document.createElement("div");
    const kicker = document.createElement("span");
    kicker.className = "services-area-kicker";
    kicker.textContent = area.name;

    const title = document.createElement("h3");
    title.textContent = `${area.totalItems} serviços nessa área`;

    headingWrap.append(kicker, title);

    const count = document.createElement("span");
    count.className = "services-area-count";
    count.textContent = `${area.groups.length} categorias`;

    header.append(headingWrap, count);
    card.appendChild(header);

    const lead = document.createElement("p");
    lead.className = "services-area-lead";
    lead.textContent = `Categorias desta área: ${area.groups.map((group) => group.name).join(", ")}.`;
    card.appendChild(lead);

    const groupsWrap = document.createElement("div");
    groupsWrap.className = "services-groups";

    area.groups.forEach((group) => {
      const groupCard = document.createElement("div");
      groupCard.className = "services-group-card";

      const groupHeader = document.createElement("div");
      groupHeader.className = "services-group-head";

      const groupTitle = document.createElement("strong");
      groupTitle.className = "services-group-title";
      groupTitle.textContent = group.name;

      const groupCount = document.createElement("span");
      groupCount.className = "services-group-count";
      groupCount.textContent = `${group.totalItems} item${group.totalItems > 1 ? "s" : ""}`;

      groupHeader.append(groupTitle, groupCount);
      groupCard.appendChild(groupHeader);

      const groupList = document.createElement("ul");
      groupList.className = "services-group-preview";

      group.preview.forEach((description) => {
        const listItem = document.createElement("li");
        listItem.textContent = description;
        groupList.appendChild(listItem);
      });

      groupCard.appendChild(groupList);
      groupsWrap.appendChild(groupCard);
    });

    card.appendChild(groupsWrap);

    const highlightWrap = document.createElement("div");
    highlightWrap.className = "services-area-highlights";

    const highlightTitle = document.createElement("span");
    highlightTitle.className = "services-area-highlight-title";
    highlightTitle.textContent = "Exemplos de atendimento";
    highlightWrap.appendChild(highlightTitle);

    const highlightList = document.createElement("ul");
    highlightList.className = "services-area-highlight-list";

    area.previewItems.forEach((description) => {
      const listItem = document.createElement("li");
      listItem.textContent = description;
      highlightList.appendChild(listItem);
    });

    highlightWrap.appendChild(highlightList);
    card.appendChild(highlightWrap);

    const actions = document.createElement("div");
    actions.className = "services-area-actions";

    const pricingLink = document.createElement("a");
    pricingLink.className = "btn-outline";
    pricingLink.href = `precos.html?area=${encodeURIComponent(area.slug)}`;
    pricingLink.textContent = "Ver preços da área";

    const contactLink = document.createElement("a");
    contactLink.className = "btn-gold";
    contactLink.href = "contato.html";
    contactLink.textContent = "Solicitar atendimento";

    actions.append(pricingLink, contactLink);
    card.appendChild(actions);
    fragment.appendChild(card);
  });

  catalog.replaceChildren(fragment);
};

const validateBudgetForm = (form) => {
  const requiredFields = [
    ["nome", "Por favor, informe seu nome."],
    ["telefone", "Informe um telefone com DDD."],
    ["servico", "Selecione o tipo de serviço."],
    ["mensagem", "Descreva brevemente o que você precisa."],
  ];
  let firstInvalid = null;

  requiredFields.forEach(([name, message]) => {
    const value = fieldValue(form, name);
    const isValid = name === "telefone" ? onlyDigits(value).length >= 10 : Boolean(value);
    setFieldState(form, name, isValid ? "" : message);

    if (!isValid && !firstInvalid) {
      firstInvalid = form.elements[name];
    }
  });

  const email = fieldValue(form, "email");
  const isEmailValid = !email || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  setFieldState(form, "email", isEmailValid ? "" : "Por favor, informe um e-mail válido.");

  if (!isEmailValid && !firstInvalid) {
    firstInvalid = form.elements.email;
  }

  return { valid: !firstInvalid, firstInvalid };
};

const buildBudgetMessage = (form) => {
  const lines = [
    "Olá, JM Edificações! Gostaria de solicitar um orçamento.",
    "",
    `Nome: ${fieldValue(form, "nome")}`,
    `Telefone/WhatsApp: ${fieldValue(form, "telefone")}`,
    fieldValue(form, "email") ? `E-mail: ${fieldValue(form, "email")}` : null,
    `Serviço desejado: ${fieldValue(form, "servico")}`,
    fieldValue(form, "localizacao") ? `Local do atendimento: ${fieldValue(form, "localizacao")}` : null,
    fieldValue(form, "prazo") ? `Prazo desejado: ${fieldValue(form, "prazo")}` : null,
    "",
    `Detalhes: ${fieldValue(form, "mensagem")}`,
    "",
    "Posso enviar fotos ou documentos por aqui, se necessário.",
  ];

  return lines.filter((line) => line !== null).join("\n").replace(/\n{3,}/g, "\n\n");
};

hydrateServiceSelects();
hydrateServicesCatalog();

document.querySelectorAll(".js-budget-form").forEach((form) => {
  const phone = form.elements.telefone;

  if (phone) {
    phone.addEventListener("input", () => {
      phone.value = formatPhone(phone.value);
    });
  }

  form.addEventListener("submit", (event) => {
    event.preventDefault();

    const validation = validateBudgetForm(form);
    const status = form.querySelector(".form-status");
    const statusText = form.querySelector(".js-form-status-text");
    const fallback = form.querySelector(".js-form-fallback");
    const button = form.querySelector(".form-submit");
    const buttonText = button ? button.querySelector("span") : null;

    if (!validation.valid) {
      validation.firstInvalid.focus();
      if (status) {
        status.classList.remove("show");
      }
      return;
    }

    const whatsappUrl = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(buildBudgetMessage(form))}`;

    if (fallback) {
      fallback.href = whatsappUrl;
      fallback.classList.add("show");
    }

    if (statusText) {
      statusText.textContent = "Mensagem preparada. O WhatsApp será aberto para concluir o envio. Se não abrir automaticamente, use o link abaixo.";
    }

    if (status) {
      status.classList.add("show");
    }

    if (button) {
      button.disabled = true;
    }

    if (buttonText) {
      buttonText.textContent = "Abrindo WhatsApp...";
    }

    window.open(whatsappUrl, "_blank", "noopener");

    window.setTimeout(() => {
      if (button) {
        button.disabled = false;
      }

      if (buttonText) {
        buttonText.textContent = "Enviar solicitação pelo WhatsApp";
      }
    }, 1400);
  });
});

const extractBackgroundImageUrl = (value) => {
  const match = value.match(/url\((['"]?)(.*?)\1\)/);
  return match ? match[2] : "";
};

const resolveMediaCaption = (element) => {
  if (element instanceof HTMLImageElement && element.alt) {
    return element.alt;
  }

  const galleryText = element.closest(".case-gallery-item")?.querySelector("div");
  if (galleryText?.textContent.trim()) {
    return galleryText.textContent.trim();
  }

  const cardTitle = element.parentElement?.querySelector("h3") || element.closest("article")?.querySelector("h3");
  if (cardTitle?.textContent.trim()) {
    return cardTitle.textContent.trim();
  }

  return "Imagem ampliada do portfólio";
};

const buildLightboxItems = () => {
  const selectors = [
    ".case-cover",
    ".case-gallery-item img",
    ".case-mini-gallery .case-gallery-item img",
    ".portfolio-feature-media",
    ".project-media",
  ];

  return Array.from(document.querySelectorAll(selectors.join(", ")))
    .map((element) => {
      const galleryItem = element.closest(".case-gallery-item");

      if (galleryItem && window.getComputedStyle(galleryItem).display === "none") {
        return null;
      }

      const source = element instanceof HTMLImageElement
        ? (element.currentSrc || element.src)
        : extractBackgroundImageUrl(element.style.backgroundImage);

      if (!source) {
        return null;
      }

      return {
        element,
        source,
        caption: resolveMediaCaption(element),
      };
    })
    .filter(Boolean);
};

const hydrateLightbox = () => {
  const items = buildLightboxItems();

  if (!items.length) {
    return;
  }

  const overlay = document.createElement("div");
  overlay.className = "lightbox";
  overlay.setAttribute("aria-hidden", "true");

  overlay.innerHTML = `
    <div class="lightbox-dialog" role="dialog" aria-modal="true" aria-label="Visualizador de imagens">
      <button class="lightbox-close" type="button" aria-label="Fechar imagem">&times;</button>
      <div class="lightbox-stage">
        <button class="lightbox-nav prev" type="button" aria-label="Imagem anterior">&#8249;</button>
        <img class="lightbox-image" alt="">
        <button class="lightbox-nav next" type="button" aria-label="Próxima imagem">&#8250;</button>
      </div>
      <div class="lightbox-caption">
        <span class="lightbox-counter"></span>
        <div class="lightbox-caption-text"></div>
      </div>
    </div>
  `;

  document.body.appendChild(overlay);

  const image = overlay.querySelector(".lightbox-image");
  const captionText = overlay.querySelector(".lightbox-caption-text");
  const counter = overlay.querySelector(".lightbox-counter");
  const prevButton = overlay.querySelector(".lightbox-nav.prev");
  const nextButton = overlay.querySelector(".lightbox-nav.next");
  const closeButton = overlay.querySelector(".lightbox-close");

  let activeIndex = 0;

  const render = () => {
    const activeItem = items[activeIndex];

    image.src = activeItem.source;
    image.alt = activeItem.caption;
    captionText.textContent = activeItem.caption;
    counter.textContent = `${activeIndex + 1} / ${items.length}`;

    const showNav = items.length > 1;
    prevButton.hidden = !showNav;
    nextButton.hidden = !showNav;
    counter.hidden = !showNav;
  };

  const open = (index) => {
    activeIndex = index;
    render();
    overlay.classList.add("open");
    overlay.setAttribute("aria-hidden", "false");
    document.body.style.overflow = "hidden";
    closeButton.focus();
  };

  const close = () => {
    overlay.classList.remove("open");
    overlay.setAttribute("aria-hidden", "true");
    document.body.style.overflow = "";
  };

  const showNext = () => {
    activeIndex = (activeIndex + 1) % items.length;
    render();
  };

  const showPrev = () => {
    activeIndex = (activeIndex - 1 + items.length) % items.length;
    render();
  };

  items.forEach((item, index) => {
    const { element } = item;

    if (element instanceof HTMLImageElement) {
      element.setAttribute("tabindex", "0");
    } else {
      element.setAttribute("tabindex", "0");
      element.setAttribute("role", "button");
    }

    element.setAttribute("aria-label", `Ampliar imagem: ${item.caption}`);

    element.addEventListener("click", () => open(index));
    element.addEventListener("keydown", (event) => {
      if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        open(index);
      }
    });
  });

  prevButton.addEventListener("click", showPrev);
  nextButton.addEventListener("click", showNext);
  image.addEventListener("click", () => {
    if (items.length > 1) {
      showNext();
    }
  });
  closeButton.addEventListener("click", close);

  overlay.addEventListener("click", (event) => {
    if (event.target === overlay) {
      close();
    }
  });

  document.addEventListener("keydown", (event) => {
    if (!overlay.classList.contains("open")) {
      return;
    }

    if (event.key === "Escape") {
      close();
    }

    if (event.key === "ArrowRight") {
      showNext();
    }

    if (event.key === "ArrowLeft") {
      showPrev();
    }
  });
};

hydrateLightbox();
